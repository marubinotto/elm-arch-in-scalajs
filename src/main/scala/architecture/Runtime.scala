...
class Runtime[Model, Msg](app: App[Model, Msg]) {
  ...
  def start(container: Element): IO[Unit] = for {
    ...
    messageProcessor <- processMessages(modelRef, msgQueue, subRef).start
    renderer <- renderLoop(modelRef, container, currentVNodeRef).start
  ...
  def dispatch(msg: Msg): IO[Unit] = {
    msgQueueOpt match {
      case Some(queue) => queue.offer(msg)
      case None =>
        IO.raiseError(
          new IllegalStateException("Runtime not started - call start() first")
        )
    }
  }
}
